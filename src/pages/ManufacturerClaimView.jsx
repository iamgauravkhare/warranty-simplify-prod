...
const ManufacturerClaimView = () => {
  ...
  const fetchClaimData = (claimId) => async (dispatch) => {
    try {
      ...
      const response = await APIHandler(
        "POST",
        "/manufacturer-services/get-claim-data",
        { claimId },
        {
  ...
  const claimApprovedHandler = async (claimId) => {
    const toastCtn = toast.loading("Loading...");
    try {
      const response = await APIHandler(
        "POST",
        "/manufacturer-services/approve-claim",
        { claimId },
        {
          Authorization: `Bearer ${token}`,
        }
      ...
      {loading ? (
        ...
        <section className="max-w-[1440px] mx-auto p-5 pt-10 text-gray-800 flex flex-col gap-5  ">
          ...
          <div className="flex flex-col gap-4 ">
            ...
            <span className="font-semibold">Damaged Product Images</span>
          ...
          <div
            ...
            onClick={() => claimApprovedHandler(claimId)}
          >
            <Button
              loading={
          ...
          {!claimData?.forwardedByRetailer && (
            <span className=" text-green-600 font-bold">
              Claim awaited to be forwaded by retailer !
            </span>
          )}
          {claimData?.progressStep == "3" && (
            <span className=" text-green-600 font-bold">
...
export default ManufacturerClaimView;
