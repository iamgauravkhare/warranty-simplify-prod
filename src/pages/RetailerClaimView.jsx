...
const RetailerClaimView = () => {
  ...
  const fetchClaimData = (claimId) => async (dispatch) => {
    try {
      ...
      const response = await APIHandler(
        "POST",
        "/retailer-services/get-claim-data",
        { claimId },
        {
  ...
  const claimForwardHandler = async (claimId) => {
    const toastCtn = toast.loading("Loading...");
    try {
      const response = await APIHandler(
        "POST",
        "/retailer-services/forward-claim",
        { claimId },
        {
          Authorization: `Bearer ${token}`,
        }
      ...
      {loading ? (
        ...
        <section className="max-w-[1440px] mx-auto p-5 pt-10 text-gray-800 flex flex-col gap-5  ">
          ...
          <div className="flex flex-col gap-4 ">
            ...
            <span className="font-semibold text-xl">Consumer Details</span>
            <span>
              <span className="font-semibold">Name : </span>
              {claimData?.consumerId?.firstname +
                " " +
                claimData?.consumerId?.lastname}
            </span>
            <span>
              <span className="font-semibold">Email : </span>
              {claimData?.consumerId?.email}
            </span>
            <span>
              <span className="font-semibold">Phone Number : </span>
              {claimData?.consumerId?.mobileNumber}
            </span>
            ...
            <span>
              <span className="font-semibold">Brand : </span>
              {claimData?.manuData?.brandname}
            </span>
            <span>
              <span className="font-semibold">Contact : </span>
              {claimData?.manuData?.manufacturerId?.email}
            </span>
            <span>
              <span className="font-semibold">Email : </span>
              {claimData?.manuData?.manufacturerId?.mobileNumber}
            </span>
          </div>
          <div
            className="w-fit my-5"
            onClick={() => claimForwardHandler(claimId)}
          >
            <Button loading={claimData?.forwardedByRetailer}>
              Forward to Manufacturer
            </Button>
          </div>
          {claimData?.forwardedByRetailer && (
            <span className=" text-green-600 font-bold">
              Claimed forwarded to manufacturer!
            </span>
          )}
        </section>
      )}
...
export default RetailerClaimView;
